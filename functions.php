...
function twitter_account() {
	return site_meta('twitter', 'weo_doodles');
}
...
/**
 * @param int $limit How many posts you want to display
 */
function my_latest_posts($limit = 3) {
    // only run on the first call
    if( ! Registry::has('my_latest_posts')) {
        // capture original article if one is set
        if($article = Registry::get('article')) {
            Registry::set('original_article', $article);
        }
    }
    if( ! $posts = Registry::get('my_latest_posts')) {
        $posts = Post::where('status', '=', 'published')->sort('created', 'desc')->take($limit)->get();
        Registry::set('my_latest_posts', $posts = new Items($posts));
    }
    if($result = $posts->valid()) {
        ...
        // remove items
        Registry::set('my_latest_posts', false);
    }
    return $result;
}
